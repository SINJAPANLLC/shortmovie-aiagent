...
- **プロダクションスタジオ** (/production/{drama_id}): 10ステップフロー制御
  - ①テーマ設定 → ②ストーリー生成 → ③シーン分割&ト書き → ④（③に統合）→ ⑤画像生成 → ⑥動画生成 → ⑦音声生成 → ⑧字幕生成 → ⑨動画結合 → ⑩投稿
  - フローステップナビゲーション（ステップ完了状態を色で表示）
  - シーンごとにキャラクター固定（scene JSON に character_id 保存）
  - シーン画像の個別/一括生成 (Luma Photon → Pollinations)
  - シーン動画の個別/一括生成 (画像→Kling AI → Luma → Ken Burns)
  - シーン別音声の個別/一括生成 + 統合音声生成 (ElevenLabs マルチボイス)
  - 字幕独立生成 (SRT) → 動画組み立て時に焼き付け
