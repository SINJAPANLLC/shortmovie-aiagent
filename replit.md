...
## Pipeline (9 Steps)
1. 初期化 → 2. テーマ生成 → 3. 脚本生成 → 4. シーン分割 → 5. 動画シーン生成(Kling) → 6. 音声生成(ElevenLabs) → 7. 動画編集(FFmpeg) → 8. 投稿(YouTube) → 9. 完了
...
app/
  ...
  templates/
    ...
    dashboard.html               # Dashboard with stats + pipeline
    dramas.html                  # Drama list
    drama_detail.html            # Drama detail view
    generate.html                # Manual generation page
    settings.html                # API settings + usage
  static/
